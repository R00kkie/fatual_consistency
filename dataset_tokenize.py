...
class textDataset(Dataset):
    def __init__(
        ...
        tokenized = []
        # tokenized['input_text'] = self.tokenizer(content, truncation=True,max_length=self.max_seq_len,pad_to_max_length=True,  return_attention_mask=True, return_tensors="pt")
        # tokenized['input_summary'] = self.tokenizer(summary, truncation=True,max_length=self.max_seq_len,pad_to_max_length=True,  return_attention_mask=True, return_tensors="pt")
        for i in tqdm(range(len(self.datas))):
            temp = {}
            temp['input_text'] = self._preprocess_title(self.datas[i]['content'])
            temp['input_summary'] = self._preprocess_title(self.datas[i]['summary'])
...
if __name__ == '__main__':
    data_loader = textSumDataModule(
        data_dir='./data/AdvertiseGen/AdvertiseGen',
        train_data_dir='./data/AdvertiseGen/AdvertiseGen/train.json',
        test_data_dir='./data/AdvertiseGen/AdvertiseGen/dev.json'
    ).setup()
    print(data_loader)